...
- Nearest `0.30` and `0.70` threshold prices are derived from `/api/risk/levels` and rendered as public callouts.
- EN/RU daily brief copy is generated by the backend and rendered by locale switch in the frontend.
- Waitlist capture stores email leads in PostgreSQL. Existing Telegram rows remain historically valid.
- Methodology documentation exists in `docs/product/risk-methodology.md`, and the public page includes a methodology section,
...
- Complete the production-pilot release gate in `docs/operations/production-readiness.md`.
