...
| Monitoring | Accepted/closed for small operator-watched pilot with limitation. | 2026-07-14 sanitized acceptance: Cloudflare Tunnel Health Alert configured, external uptime monitor provider category recorded, and public homepage availability monitoring configured. | Dedicated external `/api/health` and `/api/readiness` freshness monitors, stale-data after-window alerting, collector-failure alerts, backup freshness alerts, and explicit alert delivery evidence remain pending before broader launch. |
...
- Dedicated external `/api/health` and `/api/readiness` monitoring, stale-data after-window alerting, collector-failure
  alerts, backup freshness alerts, and explicit sanitized alert delivery evidence.
...
- Commercial/source-terms readiness, paid-plan decision, legal approval, dependency/license external confirmation, and
  project license decision if paid or broader distribution happens.
