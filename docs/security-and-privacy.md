...
privacy status as of the 2026-07-10 gap pass, local metadata implementation, focused local accessibility pass, and local
dependency/license evidence pass:
...
| Dependency and security maintenance cadence | partial; cadence documented and local dependency/license evidence recorded | The cadence is a manual monthly review of container images, Python dependencies, npm dependencies, GitHub Actions versions, advisories, vulnerability scans, and secret-scan output until automation such as Dependabot or Renovate is chosen. [Dependency and License Review](dependency-license-review.md) records the 2026-07-10 local inventory: npm lockfile license metadata is present, including `@axe-core/playwright` and `axe-core` as `MPL-2.0`; Python package license metadata, transitive Python inventory, container image/OS package licenses, GitHub Actions/license posture, vulnerability/advisory checks, project license choice, and legal compatibility remain external/manual checks. |
